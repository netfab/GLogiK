...
#ifndef __GLOGIK_GKDBUS_EVENT_TWO_STRINGS_TO_STRING_H__
#define __GLOGIK_GKDBUS_EVENT_TWO_STRINGS_TO_STRING_H__
...
{
	public:
		TwoStringsToStringEvent(
			const char* n,
			const std::vector<DBusMethodArgument> & a,
			std::function<const std::string(const std::string&, const std::string&)> c,
			GKDBusEventType t,
			const bool i
			)	: GKDBusEvent(n, a, t, i), callback(c) {}
		~TwoStringsToStringEvent() = default;
...
class EventTwoStringsToString
{
	public:
		void addTwoStringsToStringEvent(
	...
	protected:
		EventTwoStringsToString() = default;
		virtual ~EventTwoStringsToString() = default;
