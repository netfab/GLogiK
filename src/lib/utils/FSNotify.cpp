...
FSNotify::~FSNotify()
{
	...
	/* closing queue */
	if( (_inotifyQueueFD > -1) and (close(_inotifyQueueFD) == -1) )
	{
	...
	for (;;)
	{
		...
		for (ptr = buf; ptr < buf + len; ptr += sizeof(struct inotify_event) + event->len)
		{
			...
			if (event->len)
			{
				...
				erase_if(
					filesMap,
					[&name]( auto & item ) -> const bool { return item.second != name; }
				);
			}
			else
			{ /* watched object event */
				filesMap.clear();
...
const int FSNotify::addNotifyWatch(const std::string & path, const std::uint32_t & mask)
{
	...
	if(it == _watchedDescriptorsMap.end())
	{
		...
		_watchedDescriptorsMap.insert(
			std::pair<const std::string,
			WatchedObject>(path, WatchedObject(ret))
		);
